import{S,b as d}from"./assets/vendor-D1F99FMn.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))o(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(s){if(s.ep)return;s.ep=!0;const a=n(s);fetch(s.href,a)}})();const r={get mainPage(){return document.querySelector(".main-page")},get mainScreen(){return document.querySelector(".main-screen")},get taskScreen(){return document.querySelector(".section-task")},get notesList(){return document.querySelector(".notes-list")},get createBtn(){return document.querySelector(".create-btn")},get listTask(){return document.querySelector(".list-task")},get formTask(){return document.querySelector(".controls-form")},get inputTask(){return document.querySelector(".task-input")},get removeBtn(){return document.querySelector(".remove-btn")},get saveBtn(){return document.querySelector(".save-btn")},get title(){return document.querySelector(".title")},get toMenuBtn(){return document.querySelector(".to-menu-btn")},themeBtn:document.querySelector(".theme-btn")};function l(e,t){try{localStorage.setItem(e,JSON.stringify(t))}catch(n){console.log(n)}}function k(e){try{const t=JSON.parse(localStorage.getItem(e));return t!==null?t:null}catch(t){console.log(t)}}let u=null;function L(e){const t=e.target.closest(".notes-btn");if(e.target.closest(".delete-note-btn")&&y("Бажаєте видалити це завдання?",()=>{const o=e.target.closest(".delete-note-btn").closest(".notes-list-item");if(!o)return;const s=Number(o.querySelector(".notes-btn").dataset.id),a=c.findIndex(i=>i.taskId===s);a!==-1&&c.splice(a,1),l("taskStorage",c),b()}),t){const n=Number(t.dataset.id);u=n;const o=c.find(s=>s.taskId===n);if(!o)return;r.mainPage.innerHTML=`<section class="section-task">
    <div class="container notes">
      <h1 class="title">${h(o.title)}</h1>
    ...
    `).join(""),r.listTask.querySelectorAll(".list-item").forEach(t=>{const n=Number(t.dataset.id),o=e.notes.find(s=>s.id===n);o&&(t.querySelector(".task-text").textContent=o.text)}))}function q(e){const t=e.target.closest(".remove-smile");if(console.log(e.target),!t)return;const n=t.closest(".list-item"),o=Number(n.dataset.id);y("Бажаєте видалити це завдання?",()=>{g(o),m()})}function g(e){const t=c.find(n=>n.taskId===u);t&&(t.notes=t.notes.filter(n=>n.id!==e),l("taskStorage",c))}function y(e,t){const n=d.create(`<h2 class="modal-title">${e}</h2>
    <div class="btn-box">
    ...
    </form>`);t.show();const n=t.element().querySelector(".task-name-input");setTimeout(()=>{n.focus()},0),t.element().querySelector(".title-task-btn").addEventListener("click",s=>{s.preventDefault();const i=t.element().querySelector(".task-name-input").value.trim();i&&(c.push({taskId:Date.now(),title:i,notes:[]}),l("taskStorage",c),t.close(),b())},{once:!0})}r.toMenuBtn.addEventListener("click",C);r.themeBtn.addEventListener("click",I);document.body.className=k("theme");function C(e){b()}function I(e){const t=d.create(`<div class="theme-btn-wrapper">
